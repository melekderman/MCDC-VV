...
for i in range(N):
    ...
    # Fission
    pcm = ax1.pcolormesh(
        YZ_Y, YZ_Z, fission, cmap=cmap, vmin=fissions.min(), vmax=fissions.max()
    )
    ...
    pcm = ax2.pcolormesh(
        YZ_Y, YZ_Z, flux, cmap=cmap, norm=colors.LogNorm(vmin=vmin, vmax=vmax)
    )
